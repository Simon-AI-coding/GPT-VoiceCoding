...
**Amended by [#275](https://github.com/Simon-AI-coding/GPT-VoiceCoding/issues/275): that same value, read with no inherited `PATH`, so it states the profile and nothing about the launch.** `LoginShellPath` runs the shell as a child of the app, and zsh's profile *prepends* to the `PATH` it inherits — so the answer was `<profile dirs> + <whatever the app already had>`, and the app's own `PATH` is a property of how it was started, not of the machine. Measured on the reference machine: a terminal-launched app rendered a plist carrying that terminal's trailing entries, a Finder-launched one rendered the same plist without them, each launch called the other's render "a job this build would write differently", and `status` reported the LaunchAgent `stale` on a machine that was current. The reader now gives the child an environment with **no `PATH` at all**; `/etc/zprofile` rebuilds the system part through `path_helper` and the user's own files prepend on top, and the answer is byte-identical to the Finder render. This removes an input rather than supplying one, so [#38](https://github.com/Simon-AI-coding/GPT-VoiceCoding/issues/38) is untouched, and "one reading, one implementation" holds: both `ProcessLauncher` and `InstallationRunner` get the same machine-determined answer. The rendered `PATH` stays the whole profile `PATH` and is not narrowed to the executable's directory — the shared app-server runs the user's Sessions' shell commands under it.

**And the CLI still reads no login shell — but it no longer renders over the terminal's own `PATH` either.** This paragraph accepted a false `stale`: `bridge-install status` compared against the terminal it was typed in, so a terminal whose `PATH` differed from the profile's reported "would write differently" about a plist that was correct, and the note was made to name the differing top-level key so it was explainable at a glance.

**Amended by [#327](https://github.com/Simon-AI-coding/GPT-VoiceCoding/issues/327): that acceptance is retired, because the same render was also being written.** What this paragraph reasoned about is `status`, which is documented as *say what is on this machine, and write nothing*. `reconcile` and `install` render from the same caller-dependent `PATH` and put the result on disk, and that was never argued anywhere. Measured on the reference machine on 2026-09-11: a terminal-run `reconcile` replaced the job's `PATH` with the profile's own entries followed by that session's — duplicated directories, plus two version-pinned plugin-cache paths that the next plugin upgrade deletes — and the `status` line above is what tells a reader to run it. Nothing was lost on that machine, and neither guard was one: `reconcile` declines to reload a job with live Sessions under it, and the app's next launch reconcile restored the file.

The rule now is that the rendered `PATH` has two sources and no third — **what the caller states, else what the standing job records** — and the process's own `PATH` is not one of them. The shell states its login-shell reading on the installation subprocess under `GPT_VOICECODING_LOGIN_PATH`, and only when the reading was actually taken; a run nobody stated one to defers to the `PATH` the plist already carries, which is what that one reading last wrote and so is no second copy of the profile. This side still reads no shell, nothing constant is introduced (#38 untouched), and the `PATH` is still the whole profile one rather than the executable's directory. `_differing_keys`' precision stays — it is useful for every other key — and the `which` that picks the executable runs over the same machine `PATH`, because a terminal choosing what goes into `ProgramArguments` is this defect one key across.

A third state falls out of the rule, and it is a refusal rather than a report. A run that was stated nothing and finds no standing job to read cannot say what this machine's `PATH` is, so it has no render to compare against and none to write: it says so and the run is **non-zero** (Simon's ruling on this ticket's review, 2026-09-11). [#276](https://github.com/Simon-AI-coding/GPT-VoiceCoding/issues/276)'s "`ok` stays true even when the `PATH` is the reason" is not reopened by that — it ruled on *this machine's `PATH` is known and has no codex on it*, which is a fact about the machine, and this is a caller with no standing over the question at all. The two are told apart by `Resolution.path_unknown` rather than by matching on a sentence.

Two further consequences, both ruled rather than assumed. A terminal-run `status` on a current machine now reports **`current`**: a terminal has no standing to judge the profile `PATH`, the authority on whether it changed is the shell's reading at every app launch, and the alternative — a "this key cannot be judged from here" sentence on every run — was considered and declined as permanent noise (Simon's ruling, 2026-09-11). And checking a real machine's installation state now takes only the installed interpreter — `/Applications/GPT-VoiceCoding.app/Contents/Resources/engine/bin/python3 -m gpt_voicecoding.installation status`, because the hook block names the interpreter that wrote it (#38) — rather than that **and** a terminal whose `PATH` is the profile's.
...
**ADR 0020 is not reopened.** Its rule — a Codex Session is a daemon-held user root that a live terminal vouches for — is unchanged. What changed is who started the server holding those threads; see the vocabulary amendment at the head of that document. [#144](https://github.com/Simon-AI-coding/GPT-VoiceCoding/issues/144)'s identity rule is untouched, and so is [#132](https://github.com/Simon-AI-coding/GPT-VoiceCoding/issues/132)'s plist-SHA read-back.
...
1. **A Live Call on the resolved executable.** Needs a person and a microphone; also [#270](https://github.com/Simon-AI-coding/GPT-VoiceCoding/issues/270)'s scenario.
