...
**And the CLI still reads no login shell, which is what a `stale` from a terminal means.** `bridge-install status` compares against the terminal it was typed in, so a terminal whose `PATH` differs from the profile's reports "would write differently" about a plist that is correct. That is accepted; the note now names the differing top-level key (`EnvironmentVariables.PATH`) so it is explainable at a glance. Checking a real machine's installation state therefore takes both the installed interpreter — `/Applications/GPT-VoiceCoding.app/Contents/Resources/engine/bin/python3 -m gpt_voicecoding.installation status`, because the hook block names the interpreter that wrote it (#38) — **and** a terminal whose `PATH` is the profile's.
